...
/// The locking process is retried. A second failure is slot-fatal on legacy paths,
/// but MCP-active slots may deterministically skip only self-conflicting entries.
...
fn queue_batches_with_lock_retry(
    ...
    match first_err(&lock_results) {
        ...
        Err(err) => {
            ...
            let mcp_slot_active = bank
                .feature_set
                .activated_slot(&feature_set::mcp_protocol_v1::id())
                .is_some_and(|activated_slot| bank.slot() >= activated_slot);
            let mcp_lock_conflict = matches!(
                err,
                TransactionError::AccountInUse | TransactionError::AlreadyProcessed
            );

            if mcp_slot_active && mcp_lock_conflict {
                warn!(
                    "MCP replay skipped self-conflicting entry: slot={}, starting_index={}, err={:?}",
                    bank.slot(),
                    starting_index,
                    err,
                );
                datapoint_error!(
                    "validator_process_entry_mcp_skip",
                    ("slot", bank.slot() as i64, i64),
                    ("starting_index", starting_index as i64, i64),
...
pub mod tests {
    ...
    fn test_process_entries_2nd_entry_collision_with_self_and_error(
        ...
        bank.deactivate_feature(&feature_set::mcp_protocol_v1::id());
    ...
    #[test_case(true; "mcp")]
    fn test_second_lock_retry_account_in_use_skip_behavior(mcp_enabled: bool) {
        agave_logger::setup();
        ...
        assert_matches!(bank.transfer(5, &mint_keypair, &sender.pubkey()), Ok(_));

        let entry_ok = next_entry(
            &bank.last_blockhash(),
            1,
            vec![system_transaction::transfer(
                &sender,
        ...
        let entry_self_conflict = next_entry(
            &entry_ok.hash,
            1,
            vec![
                system_transaction::transfer(
        ...
        let result =
            process_entries_for_tests_without_scheduler(&bank, vec![entry_ok, entry_self_conflict]);

        assert_eq!(bank.get_balance(&sender.pubkey()), 4);
        assert_eq!(bank.get_balance(&recipient_a.pubkey()), 1);
        assert_eq!(bank.get_balance(&recipient_b.pubkey()), 0);
        if mcp_enabled {
            assert_eq!(result, Ok(()));
        } else {
            assert_eq!(result, Err(TransactionError::AccountInUse));
    ...
    #[test_case(false; "legacy")]
    #[test_case(true; "mcp")]
    fn test_second_lock_retry_already_processed_skip_behavior(mcp_enabled: bool) {
        agave_logger::setup();
        ...
        let duplicate_entry = next_entry(
            &bank.last_blockhash(),
            1,
            vec![
                system_transaction::transfer(
        ...
        let result = process_entries_for_tests_without_scheduler(&bank, vec![duplicate_entry]);

        assert_eq!(bank.get_balance(&keypair1.pubkey()), 5);
        assert_eq!(bank.get_balance(&keypair2.pubkey()), 5);
        if mcp_enabled {
            assert_eq!(result, Ok(()));
        } else {
            assert_eq!(result, Err(TransactionError::AlreadyProcessed));
